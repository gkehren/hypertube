...
| --- | --- | --- | --- | --- |
| UI | Slint shell, torrent table, multi-selection, sorting, details, search, preferences, and logs | Implemented | `src/ui/slint/SlintAppController.cpp`, `ui/` | Screen-reader and platform runtime validation remain ongoing. |
| UI | File, magnet, preferences, search, and torrent selection keyboard shortcuts | Implemented | Slint callbacks and `SlintAppController` | Ctrl/Cmd-click toggles, Shift-click selects a range, and Ctrl/Cmd+A selects visible torrents. |
...
| Torrents | Pause, resume, force-start, recheck, queue, reannounce, sequential, file priority, speed limits, and removal | Implemented | `SlintAppController`, `TorrentManager` | Selected torrents support bulk actions with per-torrent failure reporting; removal behavior depends on the selected mode. |
...
- onboarding and richer notifications;
