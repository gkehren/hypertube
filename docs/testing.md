...
| `slint_model_tests` | Slint torrent/search model reconciliation, stable IDs, and large-model updates (Slint builds only). |
| `slint-preview-check` | Compiles the static Slint preview sources with the pinned compiler (Slint builds only). |
...
ctest --test-dir build-slint -R slint-preview-check --output-on-failure
```

To validate frontend dependency isolation, configure all three supported
variants:
